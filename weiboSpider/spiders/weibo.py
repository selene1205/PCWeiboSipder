...
class WeiboSpider(Spider):
    ...
    def start_requests(self):
        ...
        start_time = datetime.datetime.strptime("2014-09-05-0", "%Y-%m-%d-%H")
        while start_time < datetime.datetime.strptime("2017-12-25-0", "%Y-%m-%d-%H"):
            if start_time < datetime.datetime.strptime("2016-1-1-0", "%Y-%m-%d-%H") or datetime.datetime.strptime(
                    "2017-1-1-0", "%Y-%m-%d-%H") < start_time < datetime.datetime.strptime("2017-8-1-0", "%Y-%m-%d-%H"):
                temp_time = start_time + datetime.timedelta(days=20)
                start_time_str = start_time.strftime("%Y-%m-%d")
                temp_time_str = temp_time.strftime("%Y-%m-%d")

                url = 'http://s.weibo.com/weibo/{}&scope=ori&suball=1&timescope=custom:{}:{}&nodup=1'.format(keyword,
                    start_time_str, temp_time_str)
                yield Request(url=url, callback=self.parse, meta={"flag": 1},
                              )
                start_time = temp_time + datetime.timedelta(days=1)
            else:
                temp_time = start_time + datetime.timedelta(hours=3)
                start_time_str = start_time.strftime("%Y-%m-%d-%H")
                temp_time_str = temp_time.strftime("%Y-%m-%d-%H")
                url = 'http://s.weibo.com/weibo/{}&scope=ori&suball=1&timescope=custom:{}:{} '.format(keyword,
                    start_time_str, temp_time_str)
                yield Request(url=url, callback=self.parse, meta={"flag": 1},
                              )
                start_time = temp_time + datetime.timedelta(hours=1)
        # text_url = 'http://s.weibo.com/weibo/e%25E7%25A7%259F%25E5%25AE%259D&typeall=1&suball=1&timescope=custom:2014-07-15:2014-07-15'
        # yield Request(url=text_url, callback=self.parse, meta={"flag": 1},
        #               dont_filter=True)
