...
class WeiboSpider(Spider):
    ...
    def start_requests(self):
        url = 'http://s.weibo.com/weibo/{}&scope=ori&suball=1&timescope=custom::{}'.format(keyword,'2015-04-30')
        yield Request(url=url, callback=self.parse, meta={"flag": 1},
                      )
        start_time = datetime.datetime.strptime("2015-04-30", "%Y-%m-%d")
        ...
        if flag:
            ...
            if len(ul) >=49 and response.meta.get('start_time'):
                ...
                for i in range(24):
                    ...
                    yield Request(url=xiaoshi_url, callback=self.parse, meta={"flag": 0, })
                return
            else:
                for li in ul:
